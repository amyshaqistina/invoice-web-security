...
| (RBAC)Admin dashboard manage users                                         | Admin dashboard allowed access to user management without fine-grained role checks; any authenticated admin could perform all actions. | Role-based access control (RBAC) applied. Only users with `super-admin` role can delete users or change roles; other admins have limited permissions (view/edit). Policies define access per action. | <img width="1919" height="914" alt="image" src="https://github.com/user-attachments/assets/82c60166-814e-490e-8db9-97ac0d6ec5a2" />|
